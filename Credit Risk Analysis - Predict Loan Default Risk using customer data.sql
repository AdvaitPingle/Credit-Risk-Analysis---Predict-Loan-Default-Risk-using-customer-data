...
--Step 2: Verify the Imported Data (Before diving into analysis, ensure the dataset is properly imported and structured.)
SELECT TOP 10 * FROM Loan_default

--Next Step: Start with Basic Data Exploration (Run a query to check how many loans in the dataset are defaulted versus repaid (based on the  column):
...
-- Next Step: Analyze Defaults by Region (Regions often play a critical role in loan performance. Let’s now investigate the default rates across different geographic regions () to identify high-risk locations.)
...
--Step 5: Analyze Defaults by Credit Score 
...
-- Step 6: Analyze Loan-to-Value (LTV) Ratio and Defaults
